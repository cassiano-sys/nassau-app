          ...
          <div className="card" style={{ marginBottom: 14 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {players.map((p, pi) => {
                ...
                // Score color in live input
                const scoreColor = !isSet ? 'var(--cream)' :
                  diff <= -2 ? '#ffd700' :
                  diff === -1 ? 'var(--green)' :
                  diff === 0  ? 'var(--cream)' :
                  diff === 1  ? '#e8a070' : 'var(--red)'
                        ...
                        {isSet ? (
                          <>
                            <input type="number" min="1" max="20"
                              ...
                              style={{ color: scoreColor, borderColor: isSet && diff !== 0 ? scoreColor + '66' : undefined }}
                            />
      ...
      {indivResults.map((res, mi) => {
        ...
        return (
          <div key={mi} style={{ marginBottom: 14, paddingBottom: 14, borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
            <div style={{ fontSize: 10, fontWeight: 600, color: 'var(--muted2)', marginBottom: 10, textTransform: 'uppercase', letterSpacing: '2px', display: 'flex', alignItems: 'center', gap: 6 }}>
              <span style={{ color: 'var(--gold)' }}>{players[a].name}</span>
              <span style={{ fontWeight: 400, fontSize: 9 }}>vs</span>
              <span style={{ color: 'var(--gold)' }}>{players[b].name}</span>
            </div>
