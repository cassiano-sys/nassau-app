...
export function RankingScreen({ onBack, session }) {
  ...
  // Ranking geral — agrupa por primeiro nome (resolve CASSIANO vs Cassiano)
  const ranking = useMemo(() => {
    ...
    ;(data || []).forEach(p => {
      ...
      // Mantém capitalização mais recente
      if (p.player_name && p.player_name[0] === p.player_name[0].toUpperCase() &&
          p.player_name[0] !== p.player_name[0].toLowerCase()) {
...
export function HistoryScreen({ onBack, session }) {
  ...
  const fmtMoney = (v) => !v ? 'R$ 0' : `${v > 0 ? '+' : ''}R$ ${v}`
  ...
  return (
    <div className="screen">
      ...
      <div className="screen-body">
        <div className="card" style={{ marginBottom: 14 }}>
          <div className="perf-grid">
            <div className="perf-cell">
              <div className={`perf-val ${myStats.total>0?'pos':myStats.total<0?'neg':'neu'}`}>{fmtMoney(myStats.total)}</div>
        ...
        {loading ? <div className="empty-state"><p>Carregando...</p></div>
        : rounds.length === 0 ? (
          <div className="empty-state"><div className="icon">🏌️</div><p>Nenhuma rodada encontrada.</p></div>
        ) : rounds.map(r => (
          <div key={r.id} className="hist-card">
            <div className="hist-date">{fmtDate(r.played_at)}</div>
            <div style={{ fontSize: 11, color: 'var(--muted)', marginBottom: 8 }}>{r.course_name} · {r.format?.toUpperCase()}</div>
                ...
                .map((p,i) => (
                  <div key={i} className="hist-player">
                    <span className="hist-pname" style={{
                      ...
                      {p.player_name} <small style={{ color:'var(--muted)', fontWeight:400 }}>HCP{p.handicap}</small>
                    </span>
                    <span className={p.money_result>0?'pos':p.money_result<0?'neg':'neu'}>{fmtMoney(p.money_result||0)}</span>
                  </div>
                ))}
            </div>
          </div>
